...
import { VerifyResolution } from "../src/application/verify-resolution.js";
...
function makeResolvedIssue(): Issue {
  const issue = unwrap(
    Issue.create({
  ...
  unwrap(issue.resolve("Fixed the pump", "2026-01-04T00:00:00.000Z"));
  return issue;
}
...
describe("VerifyResolution", () => {
  it("returns an error when the issue does not exist", async () => {
    ...
    const useCase = new VerifyResolution({ repository, clock, eventPublisher });
  ...
  it("verifies a resolved issue, saves it, and publishes issue.verified.v1", async () => {
    const issue = makeResolvedIssue();
    ...
    expect(eventPublisher.eventNames()).toEqual(["issue.verified.v1"]);
  ...
  it("does not save or publish when the issue is not resolved yet", async () => {
    const issue = unwrap(
      Issue.create({
        id: unwrap(issueId("issue-1")),
        villageId: unwrap(villageId("village-1")),
