...
describe("DevelopmentPlan", () => {
  ...
  it("adds a goal with area and description", () => {
    ...
    const plan = new DevelopmentPlan(planIdResult.value, villageIdResult.value);
    const goalIdResult = plan.addGoal("education", "Improve primary school infrastructure");
  ...
  it("rejects invalid goal areas", () => {
    ...
    const result = plan.addGoal("invalid_area", "Description");
  ...
  it("adds a milestone to a goal", () => {
    ...
    const goalId = goalIdResult.value;
    const milestoneResult = plan.addMilestone(goalId, "Complete roof repairs", "2026-12-31");
  ...
  it("rejects adding milestone to non-existent goal", () => {
    ...
    const plan = new DevelopmentPlan(planIdResult.value, villageIdResult.value);
    const result = plan.addMilestone("non-existent" as any, "Title", "2026-12-31");
  ...
  it("calculates overall progress across multiple goals", () => {
    ...
    const g1Result = plan.addGoal("education", "Education improvements");
    ...
    const g2Result = plan.addGoal("health", "Health improvements");
